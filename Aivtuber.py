...
parser.add_argument("--mic", type=int, default=None, help="Microphone device index (use --list-mics to see available devices)")
parser.add_argument("--list-mics", action="store_true", help="List available microphone devices and exit")
parser.add_argument("--find-working-mic", action="store_true", help="Automatically find the first working microphone")
...
class AIVtuber:
    def __init__(self):
        ...
        self.mic_index = args.mic
        if self.mic_index is not None:
            console.print(f"[cyan]Using microphone device index: {self.mic_index}[/cyan]")
...
if __name__ == "__main__":
    ...
    if args.list_mics:
        console.print("[cyan]Available input devices:[/cyan]")
    ...
    if args.find_working_mic:
        console.print("[cyan]Scanning for working microphones...[/cyan]")
        audio = pyaudio.PyAudio()
        found = None
        for i in range(audio.get_device_count()):
            info = audio.get_device_info_by_index(i)
            ...
            try:
                ...
                console.print(f"  [{i}] {info['name']} - level: {level:.1f}")
                if found is None and level > 1.0:
                    found = i
                    console.print(f"    [green]^ Working mic found![/green]")
            except Exception as e:
                console.print(f"  [{i}] {info['name']} - failed: {e}")
        audio.terminate()
        if found is not None:
            console.print(f"\n[green]Recommended: --mic {found}[/green]")
        else:
            console.print("\n[yellow]No working microphone detected.[/yellow]")
        exit(0)
